...
type (
	Ticker struct {
		...
		InstType InstrumentType `json:"instType,string"`
	...
	Trade struct {
		...
		TradeID JSONFloat64 `json:"tradeId"`
		...
		Side TradeSide `json:"side,string"`
		// ts String 成交明细产生时间，Unix时间戳的毫秒数格式，如 1597026383085
		TS JSONTime `json:"ts"`
	}
	TotalVolume24H struct {
		// volUsd String 24小时平台总成交量，以美元为单位
		VolUsd JSONFloat64 `json:"volUsd"`
		// volCny String 24小时平台总成交量，以人民币为单位
		VolCny JSONFloat64 `json:"volCny"`
		// ts String 成交明细产生时间，Unix时间戳的毫秒数格式，如 1597026383085
		TS JSONTime `json:"ts"`
	}
	...
	Component struct {
		...
		SymPx JSONFloat64 `json:"symPx"`
		// wgt String 权重
		Wgt JSONFloat64 `json:"wgt"`
		// cnvPx String 换算成指数后的价格
		CnvPx JSONFloat64 `json:"cnvPx"`
	}
)
