...
type (
	PlaceOrder struct {
		...
		SCode JSONInt64 `json:"sCode"`
		// ordId String 订单ID ordId或clOrdId至少填一个 ordId重复
		OrdID JSONFloat64 `json:"ordId"`
	}
	CancelOrder struct {
		// ordId String 订单ID ordId或clOrdId至少填一个 ordId重复
	...
	Order struct {
		...
		Px JSONFloat64 `json:"px"`
		// sz String 是 委托数量
		Sz JSONInt64 `json:"sz"`
		// pnl String 收益
		Pnl JSONFloat64 `json:"pnl"`
		// accFillSz String 累计成交数量
		AccFillSz JSONInt64 `json:"accFillSz"`
		// fillPx String 最新成交价格，如果成交数量为0，该字段也为 0
		FillPx JSONFloat64 `json:"fillPx"`
		// fillSz String 最新成交数量
		FillSz JSONInt64 `json:"fillSz"`
		// fillTime String 最新成交时间
		FillTime JSONFloat64 `json:"fillTime"`
		...
		TpTriggerPx JSONFloat64 `json:"tpTriggerPx"`
		// tpOrdPx String 止盈委托价
		TpOrdPx JSONFloat64 `json:"tpOrdPx"`
		// slTriggerPx String 止损触发价
		SlTriggerPx JSONFloat64 `json:"slTriggerPx"`
		// slOrdPx String 止损委托价
		SlOrdPx JSONFloat64 `json:"slOrdPx"`
		...
		Rebate JSONFloat64 `json:"rebate"`
		...
		TgtCcy QuantityType `json:"tgtCcy"`
	...
	TransactionDetail struct {
		...
		// tag String 否 订单标签  字母（区分大小写）与数字的组合，可以是纯字母、纯数字，且长度在1-16位之间。
		Tag JSONFloat64 `json:"tag"`
		// fillPx String 最新成交价格，如果成交数量为0，该字段也为 0
		FillPx JSONFloat64 `json:"fillPx"`
		// fillSz String 最新成交数量
		FillSz JSONFloat64 `json:"fillSz"`
		// feeCcy String 交易手续费币种
		FeeCcy JSONFloat64 `json:"feeCcy"`
		// fee String 订单交易手续费，平台向用户收取的交易手续费，手续费扣除为 负数 。如：  -0.01
		Fee JSONFloat64 `json:"fee"`
		// instType String 产品类型 SPOT ：币币 MARGIN ：币币杠杆 SWAP ：永续合约  FUTURES ：交割合约    OPTION ：期权 交易产品类型不匹配指数（instType和uly不匹配）
		InstType InstrumentType `json:"instType"`
		...
		ExecType OrderFlowType `json:"execType"`
	...
	AlgoOrder struct {
		...
		PxVar JSONFloat64 `json:"pxVar"`
		// pxSpread String 可选 挂单价距离盘口的价距
		PxSpread JSONFloat64 `json:"pxSpread"`
		// pxLimit String 是 挂单限制价
		PxLimit JSONFloat64 `json:"pxLimit"`
		// sz String 是 委托数量
		Sz JSONInt64 `json:"sz"`
		// szLimit String 是 单笔数量
		SzLimit JSONInt64 `json:"szLimit"`
		// actualSz String 实际委托量
		ActualSz JSONFloat64 `json:"actualSz"`
		// actualPx String 实际委托价
		ActualPx JSONFloat64 `json:"actualPx"`
		// pnl String 收益
		Pnl JSONFloat64 `json:"pnl"`
		// accFillSz String 累计成交数量
		AccFillSz JSONInt64 `json:"accFillSz"`
		...
		ActualSide PositionSide `json:"actualSide"`
		// posSide String 可选 持仓方向  在双向持仓模式下必填，且仅可选择  long  或  short 。 仅适用交割、永续。
		PosSide PositionSide `json:"posSide"`
		// side String 是 订单方向   buy ：买，  sell ：卖
		Side OrderSide `json:"side"`
		// ordType String 是 订单类型  market ：市价单 limit ：限价单  post_only ：只做maker单  fok ：全部成交或立即取消  ioc ：立即成交并取消剩余  optimal_limit_ioc ：市价委托立即成交并取消剩余（仅适用交割、永续） 逐仓自主划转保证金模式不支持ordType为iceberg、twap的策略委托单
		OrdType AlgoOrderType `json:"ordType"`
		// instType String 产品类型 SPOT ：币币 MARGIN ：币币杠杆 SWAP ：永续合约  FUTURES ：交割合约    OPTION ：期权 交易产品类型不匹配指数（instType和uly不匹配）
		InstType InstrumentType `json:"instType"`
		// tgtCcy String 否 市价单委托数量的类型，仅适用于 币币 市价订单 base_ccy : 交易货币 ； quote_ccy ：计价货币 买单默认 quote_ccy ， 卖单默认 base_ccy 计划委托不支持使用tgtCcy参数
		TgtCcy QuantityType `json:"tgtCcy"`
		...
		TriggerTime JSONTime `json:"triggerTime"`
	}
)
