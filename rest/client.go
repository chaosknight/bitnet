...
func NewClient(apiKey *entity.APIKey, baseURL entity.BaseURL, destination entity.Destination) *ClientRest {
	c := &ClientRest{
		APIKey:      apiKey,
		baseURL:     baseURL,
...
func (c *ClientRest) Do(method, path string, private bool, params ...interface{}) (*http.Response, error) {
	...
	if method == http.MethodGet {
		...
		if len(params) > 0 {
			paramsMap := entity.MI2MS(params[0])
	...
	if c.destination == entity.DemoServer {
